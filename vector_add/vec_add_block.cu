...
int main(void) {
    ...
    // Block size:
    int nThreadsPerBlock = 4;
    // To get at least 1 thread per array element:
    // int nBlocks = (N + nThreadsPerBlock - 1) / nThreadsPerBlock;
    // If we had less blocks (e.g. we reached the limit of blocks):
    int nBlocks = 4;
    add_vec<<<nBlocks, nThreadsPerBlock>>>(a, b, result, N);
